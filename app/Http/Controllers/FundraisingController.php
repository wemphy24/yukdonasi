...
class FundraisingController extends Controller
{
    ...
    public function store(StoreFundraisingRequest $request)
    {
        ...
        DB::transaction(function () use ($request, $fundraiser) {
            ...
            if($request->hasFile('thumbnail')) {
                ...
                $validated['thumbnail'] = $thumbnailPath; // storage/thumbnails/wemphy.png
            } 
    ...
    public function show(Fundraising $fundraising)
    {
        ...
        $goalReached = $totalDonation >= $fundraising->target_amount;

        $hasRequestWithdrawals = $fundraising->fundraising_withdrawals()->exists();

        $percentage = ($totalDonation/$fundraising->target_amount) * 100;
        if($percentage > 100) {
            $percentage = 100;
        }

        return view('admin.fundraisings.show', compact('fundraising', 'goalReached', 'percentage', 'totalDonation', 'hasRequestWithdrawals'));
    }
